    ...
    def run(self):
        try:
            ...
            if self.resume_dir:
                ...
                self.log_verbose(f"🔁 Resuming run {run_dir}")
            else:
...
class FileInputRow(QWidget):
    def __init__(self, label_text, placeholder, file_filter, is_save=False):
        super().__init__()
...
class RunHistoryItem(QWidget):
    ...
    def __init__(self, run_dir, state, parent=None):
        ...
        status_text = f"{state['status'].title()} • {len(state['completed_chunks'])}/{state['total_chunks']} chunks"
        ...
        if state["status"] != "completed":
            resume_btn = ModernButton("▶ Resume", variant="success")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # ==================== HEADER ====================
        header = QLabel("🎧 Audiobook Generator")
        ...
        # ==================== ACTION BUTTON ====================
        self.generate_btn = ModernButton("🚀 Generate Audiobook", variant="primary")
